...
interface VerifyInspectionRequest {
  Params: {
    token: string;
  };
  Body: {
...
export class AnnualInspectionController {
  private get inspectionRepo(): Repository<AnnualInspection> {
   ...
   */
  async createInspection(request: FastifyRequest<CreateInspectionRequest>, reply: FastifyReply) {
    try {
      ...
      // Validate inspector exists and is authorised
      ...
      return reply.status(201).send({
        ...
        nextSteps: {
          ...
          message: photos.length >= 3 
            ? 'Inspection is ready for submission' 
            : `Upload ${3 - photos.length} more photo(s) before submission`,
          requiredPhotos: [
   ...
   */
  async submitInspection(request: FastifyRequest<SubmitInspectionRequest>, reply: FastifyReply) {
    try {
      const userId = (request as any).user?.id;
      const { id } = request.params;

      // Find inspection with inspector details
      ...
      // Get inspector and warranty details for SMS
      const inspector = await this.userRepo.findOne({
   ...
   */
  async validateInspectionForSubmission(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
      const userId = (request as any).user?.id;
      ...
      return reply.send({
        ...
        message: isValid 
          ? 'Inspection is ready for submission' 
          : `Inspection needs ${missingPhotos} more photo(s) before submission`,
        requiredPhotos: [
          'Generator with RED LIGHT visible and operational',
          'Couplers/pads condition and security',
          'Corrosion/stone chips OR clear vehicle body'
   ...
   */
  async verifyInspection(request: FastifyRequest<VerifyInspectionRequest>, reply: FastifyReply) {
    try {
      const { token } = request.params;
      const { action, rejectionReason } = request.body;

      // Find inspection by verification token
      const inspection = await this.inspectionRepo.findOne({
        where: { 
      ...
      // Check token expiry
      if (inspection.verificationTokenExpires && inspection.verificationTokenExpires < new Date()) {
        return reply.status(400).send({ 
      ...
      if (action === 'CONFIRM') {
        ...
        inspection.verificationToken = undefined;
        inspection.verificationTokenExpires = undefined;
        ...
        // Get warranty details for response
        const warranty = await this.warrantyRepo.findOne({
          where: { id: inspection.warrantyId }
        });
   ...
   */
  async getInspection(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
      const userId = (request as any).user?.id;
  ...
  async listInspections(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      let inspections: AnnualInspection[] = [];

      if (user.isAuthorisedInspector) {
        // Inspectors can see inspections they performed or need to verify
        inspections = await this.inspectionRepo.find({
          where: { inspectorId: userId, isDeleted: false },
          order: { created: 'DESC' }
        });
      } else {
        // Regular users can see inspections for their warranties
        ...
        if (warrantyIds.length > 1) {
          inspections = await this.inspectionRepo
            .createQueryBuilder('inspection')
            .where('inspection.warrantyId IN (:...warrantyIds)', { warrantyIds })
            .andWhere('inspection.isDeleted = :isDeleted', { isDeleted: false })
            .orderBy('inspection.created', 'DESC')
            .getMany();
      ...
      // Get photo counts for each inspection
      const inspectionList = await Promise.all(
        inspections.map(async (inspection) => {
          ...
          // Get warranty details if not loaded
          let warranty;
          if ((inspection as any).warranty) {
            warranty = (inspection as any).warranty;
          } else {
          ...
          // Get inspector details if not loaded
          let inspector;
          if ((inspection as any).inspector) {
            inspector = (inspection as any).inspector;
          } else {
   ...
   */
  async getWarrantyInspectionHistory(request: FastifyRequest<{ Params: { warrantyId: string } }>, reply: FastifyReply) {
    try {
      ...
      // Get all inspections for this warranty
   ...
   * Validate submission requirements
   */
  private validateSubmissionRequirements(inspection: AnnualInspection): string[] {
    const errors: string[] = [];
    ...
    if (inspection.generatorMountedCorrectly === null || inspection.generatorMountedCorrectly === undefined) {
      errors.push('Generator mounting check is required');
    }

    if (inspection.redLightIlluminated === null || inspection.redLightIlluminated === undefined) {
      errors.push('RED LIGHT check is required');
    }

    if (inspection.couplersSecureSealed === null || inspection.couplersSecureSealed === undefined) {
      errors.push('Couplers security check is required');
    }

    if (inspection.ownerAdvisedPaintDamage === null || inspection.ownerAdvisedPaintDamage === undefined) {
      errors.push('Owner paint damage advisory confirmation is required');
    }

    if (inspection.ownerUnderstandsOperation === null || inspection.ownerUnderstandsOperation === undefined) {
      errors.push('Owner operation understanding confirmation is required');
    }

    if (inspection.corrosionFound === null || inspection.corrosionFound === undefined) {
    ...
    // Check that condition items with ISSUE have notes
    ...
    for (const { field, notes, name } of conditionFields) {
      if ((inspection as any)[field] === 'ISSUE' && !(inspection as any)[notes]) {

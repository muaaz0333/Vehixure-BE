...
export interface Warranty {
  ...
  installerId?: string; // Reference to installer user
  dateInstalled: Date;
  ...
  verificationStatus: 'DRAFT' | 'SUBMITTED' | 'PENDING_CUSTOMER_ACTIVATION' | 'VERIFIED' | 'REJECTED' | 'ACTIVE';
  ...
  customerActivationToken?: string;
  customerActivationTokenExpires?: Date;
  customerTermsAcceptedAt?: Date;
  customerTermsAcceptedIp?: string;
  activatedAt?: Date;
  activatedBy?: string;
  ...
  extensionBlockedAt?: Date;
  
  // Consolidated reminder tracking
  elevenMonthReminderSent?: Date;
  thirtyDayReminderSent?: Date;
  reminderAttempts: number;
  
  // Status
  status: 'DRAFT' | 'SUBMITTED' | 'PENDING_CUSTOMER_ACTIVATION' | 'VERIFIED' | 'REJECTED' | 'ACTIVE' | 'EXPIRED' | 'CANCELLED';
  isDeleted: boolean;
...
export const Warranty = new EntitySchema<Warranty>({
  name: 'Warranty',
  ...
  columns: {
    ...
    gracePeriodEndDate: {
      type: 'date',
      nullable: true,
      name: 'grace_period_end_date', // Explicit column name mapping
    },
    isGraceExpired: {
      type: 'boolean',
      default: false,

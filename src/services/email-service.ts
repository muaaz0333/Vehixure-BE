// Email service for ERPS reminder system
// This service handles sending automated reminder emails to customers
...
export interface UserCreationEmailOptions {
  fullName: string;
  email: string;
  password: string;
  role: string;
...
export interface PartnerAccountCreationEmailOptions {
  businessName: string;
...
export class EmailService {
  ...
  constructor() {
    ...
    // Create SMTP transporter using your Mailgun credentials
   ...
   */
  static async sendGenericUserCreationEmail(options: UserCreationEmailOptions): Promise<boolean> {
    const emailService = new EmailService();
    
    try {
      console.log(`📧 Sending welcome email to new user: ${options.email}`);
   ...
   */
  static async sendPartnerAccountCreationEmail(options: PartnerAccountCreationEmailOptions): Promise<boolean> {
    const emailService = new EmailService();
  ...
  async sendReminderEmail(options: EmailOptions): Promise<boolean> {
    try {
   ...
   */
  private generateUserCreationHtmlEmail(options: UserCreationEmailOptions): string {
...
<body>
   ...
   */
  private generateUserCreationTextEmail(options: UserCreationEmailOptions): string {
    const loginUrl = options.loginUrl || process.env.FRONTEND_URL || 'http://localhost:3000';
...
© ${new Date().getFullYear()} ERPS - Electronic Rust Protection System
   ...
   */
  private generatePartnerAccountCreationHtmlEmail(options: PartnerAccountCreationEmailOptions): string {
    const loginUrl = options.loginUrl || process.env.FRONTEND_URL || 'http://localhost:3000';
...
<body>
    <div class="container">
        ...
        <div class="content">
            <h2>🎉 Congratulations ${options.contactPerson}!</h2>
            ...
            <div class="business-info">
                <h3>🏢 Partner Account Information</h3>
            ...
            <div class="credentials-box">
                <h3>🔐 Admin User Login Credentials</h3>
   ...
   */
  private generatePartnerAccountCreationTextEmail(options: PartnerAccountCreationEmailOptions): string {
    const loginUrl = options.loginUrl || process.env.FRONTEND_URL || 'http://localhost:3000';
    const supportEmail = options.supportEmail || 'support@erps.com.au';
    const supportPhone = options.supportPhone || '1800 ERPS (1800 3777)';
...
<body>
   ...
   */
  private getWarningSection(subject: string): string {
   ...
   */
  getServiceStatus(): { provider: string; configured: boolean; fromEmail: string } {
    return {

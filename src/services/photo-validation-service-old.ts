...
export class PhotoValidationService {
  ...
  async validateInspectionPhotos(inspectionId: string): Promise<PhotoValidationResult> {
    ...
    try {
      ...
      // Get required categories for inspections
      ...
      // Get provided photos by category
      const photosResult = await client.query(`
        SELECT photo_category, COUNT(*) as photo_count
  ...
  async validateCorrosionRequirements(recordId: string, recordType: 'WARRANTY' | 'INSPECTION'): Promise<CorrosionValidationResult> {
    ...
    try {
      ...
      // Use the new database function for strict corrosion validation
      const validationResult = await client.query(
      ...
      return {
        ...
        photosProvided: validation.photos_provided >= 2, // Client requires 2 photos minimum
   ...
   */
  private async getWarrantyCategoryValidation(warrantyId: string): Promise<any> {
    const client = new Client(dbConfig);
  ...
  async updatePhotoCategory(
    ...
    try {
      ...
      // Verify category exists and is valid for record type
  ...
  async validatePhotoUpload(
    ...
    try {
      ...
      // Get current photo count for this category
      const tableName = recordType === 'WARRANTY' ? 'warranty_photos' : 'inspection_photos';
      ...
      return {
        valid,
        message: valid 
          ? `Upload allowed (${totalAfterUpload}/${max_photos} photos)`
          : `Upload would exceed limit (${totalAfterUpload}/${max_photos} photos)`,
        currentCount,

...
export class InspectionChecklistService {
   ...
   */
  async createInspectionChecklist(inspectionId: string): Promise<ChecklistItem[]> {
  ...
  async updateChecklistItem(
    ...
    try {
      ...
      // Validate that notes are provided if issue observed
   ...
   */
  async getInspectionChecklist(inspectionId: string): Promise<ChecklistItem[]> {
    const client = new Client(dbConfig);
   ...
   */
  async validateInspectionChecklist(inspectionId: string): Promise<ChecklistValidationResult> {
    const client = new Client(dbConfig);
    
    try {
      await client.connect();
      
      // Get checklist data in the format expected by the validation function
      ...
      // Use the new database function for strict validation
      ...
      return {
        ...
        validationMessage: isComplete 
          ? 'All checklist items completed with required notes'
          : `Incomplete items requiring notes: ${incompleteItems.join(', ')}`,
        totalItems,
        completedItems,
        itemsWithIssues
      };
   ...
   */
  async updateChecklistFromInspectionData(inspectionId: string, inspectionData: any): Promise<void> {
    const client = new Client(dbConfig);
    ...
    try {
      ...
      // Mapping of inspection fields to checklist items
      const fieldMapping = {
      ...
      // Update each checklist item based on inspection data
      for (const [field, itemCode] of Object.entries(fieldMapping)) {
        if (inspectionData[field] !== undefined) {
          let conditionStatus: 'PASS' | 'ISSUE_OBSERVED';
          ...
          if (typeof inspectionData[field] === 'boolean') {
            ...
            // Handle condition fields (PASS/ISSUE_OBSERVED)
            conditionStatus = inspectionData[field] === 'PASS' ? 'PASS' : 'ISSUE_OBSERVED';
            
            // Look for corresponding notes field
   ...
   */
  async getChecklistValidationSummary(): Promise<any> {
    const client = new Client(dbConfig);
   ...
   */
  async getIncompleteChecklists(): Promise<any[]> {
    const client = new Client(dbConfig);
   ...
   */
  async deleteInspectionChecklist(inspectionId: string): Promise<void> {
    const client = new Client(dbConfig);

...
export class PhotoValidationService {
   ...
   */
  basicPhotoValidation(photoData, recordType) {
    const requiredCategories = recordType === "WARRANTY" ? ["GENERATOR", "COUPLER", "CORROSION_OR_CLEAR"] : ["GENERATOR_RED_LIGHT", "COUPLERS", "CORROSION_OR_CLEAR"];
    const missingCategories = [];
    const categoryValidation = {};
    ...
    return {
      isValid,
      missingCategories,
      validationMessage: isValid ? `All required ${recordType.toLowerCase()} photos provided` : `Missing required photo categories: ${missingCategories.join(", ")}`,
      categoryValidation
    };
  }
  /**
  ...
  async validateCorrosionRequirements(recordId, recordType) {
    ...
    try {
      ...
      return {
        ...
        photosProvided,
        validationMessage: isValid ? "Corrosion validation passed" : "Corrosion found but missing required notes or photos"
      };
    } finally {
      await client.end();
    }
  ...
  async validatePhotoUpload(recordId, recordType, category, newPhotoCount) {
    ...
    try {
      ...
      return {
        valid,
        message: valid ? `Upload allowed (${totalAfterUpload}/${maxPhotos} photos)` : `Upload would exceed limit (${totalAfterUpload}/${maxPhotos} photos)`,
        currentCount,

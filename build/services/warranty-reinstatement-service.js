...
export class WarrantyReinstatementService {
   ...
   */
  async reinstateWarranty(request) {
    ...
    try {
      ...
      return {
        ...
        previousLapseDate: void 0,
        // Will be tracked in reinstatement history
   ...
   */
  async getWarrantyReinstatementHistory(warrantyId) {
    const client = new Client(dbConfig);
    try {
      await client.connect();
      const result = await client.query(`
      ...
      return result.rows.map((row) => ({
        ...
        reason: row.reason,
        previousLapseDate: row.previous_lapse_date ? new Date(row.previous_lapse_date) : void 0,
   ...
   */
  async checkReinstatementEligibility(warrantyId) {
    const client = new Client(dbConfig);
    try {
      await client.connect();
      ...
      const inspectionId = hasCompletedInspection ? inspectionResult.rows[0].id : void 0;
      return {
        eligible: true,
        reason: hasCompletedInspection ? "Eligible for reinstatement with completed inspection" : "Eligible for reinstatement (admin discretion)",
        hasCompletedInspection,
        inspectionId,
        lapsedDate: warranty.warranty_lapsed_at ? new Date(warranty.warranty_lapsed_at) : void 0
      };
    } finally {
      await client.end();
    }
   ...
   */
  async getReinstatementStatistics() {
    const client = new Client(dbConfig);
    try {
      await client.connect();
      const result = await client.query(`
   ...
   */
  async bulkReinstateWarranties(warrantyIds, reinstatedBy, reason) {
    const successful = [];
    const failed = [];

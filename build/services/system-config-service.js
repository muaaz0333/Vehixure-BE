...
export class SystemConfigService {
  ...
  async initializeERPSDefaults() {
    ...
    try {
      ...
      console.log("\u{1F527} Initializing ERPS system configuration based on client specifications...");
      const erpsConfigs = [
      ...
      `);
      console.log("\u{1F4CA} ERPS System Configuration Summary:");
  ...
  async updateConfig(configId, updateData) {
    ...
    try {
      ...
      const params = [];
      let paramIndex = 1;
      if (updateData.configName !== void 0) {
   ...
   */
  async toggleStatus(configId, isActive) {
    const client = new Client(dbConfig);
    try {
      await client.connect();
      const result = await client.query(`
  ...
  async getConfigValue(category, key) {
    ...
    try {
      ...
      return row.string_value || row.integer_value || row.boolean_value || row.date_value || row.json_value;
    } finally {
      await client.end();
    }
  }
  ...
  async getCategoryConfig(category) {
    ...
    try {
      ...
      return result.rows.map((row) => ({
        ...
        value: row.string_value || row.integer_value || row.boolean_value || row.date_value || row.json_value,

...
export class PartnerAccountController {
  ...
  async createPartnerAccount(request, reply) {
    try {
  ...
  async getPartnerAccounts(request, reply) {
    try {
      this.initializeRepositories();
      const { page = "1", limit = "10", search, status } = request.query;
      const skip = (parseInt(page) - 1) * parseInt(limit);
      const queryBuilder = this.partnerAccountRepository.createQueryBuilder("account").where("account.isDeleted = :isDeleted", { isDeleted: false }).orderBy("account.created", "DESC");
      if (search) {
      ...
      const [accounts, total] = await queryBuilder.skip(skip).take(parseInt(limit)).getManyAndCount();
      return reply.send({
        success: true,
  ...
  async getPartnerAccountById(request, reply) {
    try {
      this.initializeRepositories();
  ...
  async updatePartnerAccount(request, reply) {
    try {
      this.initializeRepositories();
      const { accountId } = request.params;
  ...
  // Create partner user
  async createPartnerUser(request, reply) {
    try {
      this.initializeRepositories();
      const { accountId } = request.params;
      const {
      ...
      try {
        const roleDisplayName = partnerRole === "ACCOUNT_ADMIN" ? "Account Admin" : partnerRole === "ACCOUNT_STAFF" ? "Account Staff" : "Account Installer";
  ...
  // Get partner users
  async getPartnerUsers(request, reply) {
    try {
      this.initializeRepositories();
      const { accountId } = request.params;
      const { role } = request.query;
      const queryBuilder = this.userRepository.createQueryBuilder("user").where("user.partnerAccountId = :accountId", { accountId }).andWhere("user.isDeleted = :isDeleted", { isDeleted: false }).orderBy("user.created", "DESC");
      if (role) {
  ...
  async deletePartnerAccount(request, reply) {
    try {
      this.initializeRepositories();
      const { accountId } = request.params;
      const account = await this.partnerAccountRepository.findOne({
      ...
      account.deletedAt = /* @__PURE__ */ new Date();
